...
function getMarkdown(editor: ReturnType<typeof useEditor>): string {
  ...
  if (storage && typeof (storage as MarkdownStorage).getMarkdown === 'function') {
...
function LinkInput({ onConfirm, onCancel }: LinkInputProps) {
  const [url, setUrl] = useState('')
  return (
    <div className="flex items-center gap-2 px-2 py-1 bg-white border border-gray-300 rounded shadow-sm">
      <input
        autoFocus
        type="url"
...
export function TiptapEditor({ value, onChange }: TiptapEditorProps) {
  ...
  if (!editor) {
    return <div className="w-full h-48 border rounded bg-gray-50 animate-pulse" />
  }

  return (
    <div className="border rounded-lg overflow-hidden bg-white">
      {/* ── Toolbar ─────────────────────────────────────────────────────── */}
      <div className="flex flex-wrap items-center gap-1 px-2 py-1 border-b bg-gray-50 text-xs">
        {/* Text format */}
        <ToolbarButton
          title="Bold (Ctrl+B)"
        ...
        {/* Source toggle — pushed to the right via margin-left auto */}
      ...
      {showSource ? (
        <textarea
          className="w-full font-mono text-sm p-3 min-h-[12rem] outline-none resize-y"
          value={sourceText}
